...
const Inscroll = () => {
    ...
    useEffect(() => {
        if (page > 0)
            setImage(prev => [...prev, ...data.slice(page * 15, page * 15 + 15)]);
    }, [page])
    ...
    return (
        <div>
            ...
            <div className="flex flex-wrap">
                {image != undefined && image.map((elem) => {
                    return <div className="p-1 m-4" key={elem.id}>
                        <img src={elem?.url} className="w-56"></img>
                    </div>
                })
