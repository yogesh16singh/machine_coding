...
const Folder = ({ data }: any) => {
    ...
    return (
        <div className='m-2 pl-2 '>
            <div onClick={() => openFolder()} className={`cursor-pointer flex items-center rounded-md justify-between sm:w-2/3 md:w-1/3 ${info.isFolder ? "bg-slate-400" : "bg-slate-200"}`} >
                <div className={`p-1  `}>
            ...
            </div>
            {
                showInput.show && <div>
                    ...
                    <input type='text' className='ml-4 mt-2 p-1 w-1/3 bg-slate-300 rounded-md' autoFocus onKeyDown={(e) => addnew(e)}></input>
                </div>
            }
